/**
 * Update Logic Question Controller
...
/**
 * Formats the response for successful question update
...
/**
 * Formats the response for error in question update
...
/**
 * Controller to update a logic question
 * @param {Object} req - Express request object
 * @param {Object} res - Express response object
 * @returns {Object} Response with updated question or error
 */
const updateLogicQuestionController = async (req, res) => {
  try {
    const { id } = req.params;
    const result = await updateQuestion(id, req.body);

    if (!result.success) {
      logger.warn(`Failed to update logic question with ID ${id}:`, result.errors);
    ...
    logger.error('Error in update logic question controller:', error);
