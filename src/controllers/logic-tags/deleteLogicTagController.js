...
/**
 * Controller to delete a logic tag
 * @param {Object} req - Express request object
 * @param {Object} res - Express response object
 * @returns {Object} Response with deleted tag or error
 */
const deleteLogicTagController = async (req, res) => {
  try {
    const { id } = req.params;
    const result = await deleteTag(id);

    if (!result.success) {
      logger.warn(`Failed to delete logic tag with ID ${id}:`, result.errors);
    ...
    logger.error('Error in delete logic tag controller:', error);
